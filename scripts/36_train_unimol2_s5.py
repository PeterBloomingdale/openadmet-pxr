"""
UniMol v1 fine-tuning — fifth run at LR=1e-3 with smaller batch (batch_size=4).

Diversity rationale:
  Two orthogonal changes from s4:
  1. LR=1e-3 (20× higher than s1) — pushes into a more extreme fine-tuning regime.
  2. batch_size=4 (vs 8) — smaller batches produce noisier gradient estimates, which
     randomizes the optimization path independently of the learning rate change.

  The combination of a higher LR and smaller batch means each weight update step is
  both larger and noisier than any prior run, maximizing divergence from s1/s3/s4
  while early stopping (patience=5) prevents runaway divergence.

  After training: compute pearsonr(oof_s5, oof_s1) and pearsonr(oof_s5, oof_s3/s4).
  If all < 0.95 → genuinely diverse, add to ensemble.
  If model diverged badly (OOF MAE >> 0.55) → do not add; report in decision log.

Outputs:
  models/unimol2_s5/oof_predictions.npy  (4135,)
  models/unimol2_s5/test_predictions.npy (513,)
  models/unimol2_s5/metrics.json
"""
...
_DEVICE_PARAMS = get_unimol_device_params(base_batch_size=4)  # small batch kept for diversity
...
UNIMOL_BASE_PARAMS = {
    ...
    "model_name":     "unimolv1",
    "epochs":         25,
    "learning_rate":  1e-3,   # 20× higher than s1 — aggressive; early stopping guards divergence
    "batch_size":     _DEVICE_PARAMS["batch_size"],  # small batch kept intentionally for diversity
    "early_stopping": 5,
...
def main() -> None:
    logger.info("=== UniMol Fine-tuning — Fifth Run (LR=1e-3, batch=4, unimol2_s5) ===")
    log_device_info()

    out_dir = Path("models/unimol2_s5")
    ...
    # Diversity check vs all existing UniMol runs
    for tag, path in [
        ("s1", "models/unimol2/oof_predictions.npy"),
        ("s3", "models/unimol2_s3/oof_predictions.npy"),
        ("s4", "models/unimol2_s4/oof_predictions.npy"),
    ]:
        p = Path(path)
        if p.exists():
            r = pearsonr(oof, np.load(p)).statistic
            logger.info(f"Pearson r(s5, {tag}) = {r:.4f}  (want < 0.95 for genuine diversity)")
    ...
    logger.info(
        "\nIf r(s5, s1/s3/s4) all < 0.95 AND OOF MAE < 0.55, add to scripts/11_ensemble.py:\n"
