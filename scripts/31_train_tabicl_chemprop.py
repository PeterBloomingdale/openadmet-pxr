"""
TabICL on PXR-fine-tuned Chemprop embeddings + CheMeleon + ECFP4 + rdkit2d → PCA-256.

Why this is different from scripts/26_train_tabicl.py:
  The existing tabicl uses FROZEN CheMeleon embeddings (pretrained on generic ChEMBL data).
  This script ALSO adds PXR-fine-tuned Chemprop fingerprints (300-d) from our already-
  trained Chemprop model (models/chemprop/). These fingerprints encode which structural
  features specifically matter for PXR binding — analogous to Jeremy's HTS-pretrained
  CheMeleon approach, but using our own PXR-specific fine-tuned backbone.

Feature pipeline:
  CheMeleon(2048) + Chemprop-FT(300) + ECFP4(2048) + rdkit2d(208) = 4604-d
  → VarianceThreshold(0.01) → PCA-256 (99.9%+ variance retained)

Important: Chemprop-FT embeddings are HONEST OOF (extracted via scripts/30_).
  For fold k, embeddings came from models NOT trained on fold k.

Outputs:
  models/tabicl_chemprop/oof_predictions.npy  (4135,)
  models/tabicl_chemprop/test_predictions.npy (513,)
  models/tabicl_chemprop/metrics.json
"""
...
def build_features() -> tuple[np.ndarray, np.ndarray]:
    """Load CheMeleon + Chemprop-FT + ECFP4 + rdkit2d, reduce to PCA-N_PCA."""
    logger.info("Loading features: CheMeleon + Chemprop-FT + ECFP4 + rdkit2d")
    ...
    train_cmp = np.load("data/features/train_chemprop_finetuned_emb.npy").astype(np.float32)
    test_cmp = np.load("data/features/test_chemprop_finetuned_emb.npy").astype(np.float32)
    ...
    X_train_raw = np.concatenate(
        [train_emb, train_cmp, X_all[:, ecfp4_mask], X_all[:, rdkit_mask]], axis=1
    )
    X_test_raw = np.concatenate(
        [test_emb, test_cmp, X_all_test[:, ecfp4_mask], X_all_test[:, rdkit_mask]], axis=1
    )

    logger.info(
        f"Raw features: CheMeleon={train_emb.shape[1]}, Chemprop-FT={train_cmp.shape[1]}, "
...
def main() -> None:
    logger.info("=== TabICL-Chemprop (CheMeleon + Chemprop-FT + ECFP4 + rdkit2d → PCA-256) ===")
    logger.info(f"TabICL n_estimators={N_ESTIMATORS}, PCA={N_PCA}")

    out_dir = Path("models/tabicl_chemprop")
    ...
    for path in ["data/features/train_chemeleon_emb.npy", "data/features/train_chemprop_finetuned_emb.npy"]:
