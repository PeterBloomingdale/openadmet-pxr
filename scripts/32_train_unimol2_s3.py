"""
UniMol v1 fine-tuning — third run with higher learning rate (LR=2e-4).

Why this differs from unimol2 (LR=5e-5):
  Higher LR drives more aggressive fine-tuning from the pretrained checkpoint.
  The training dynamics and convergence path differ, producing a model with
  correlated but non-identical predictions (unlike the deterministic s2 duplicate).
  unimol_tools is fully deterministic with the same settings — only changing
  LR guarantees genuinely different model weights and predictions.

Learning rate choice:
  LR=5e-5 (current):  conservative, standard for transformer fine-tuning
  LR=2e-4 (this run): 4x higher, faster adaptation, early stopping prevents overfit

Outputs:
  models/unimol2_s3/oof_predictions.npy  (4135,)
  models/unimol2_s3/test_predictions.npy (513,)
  models/unimol2_s3/metrics.json
"""
...
UNIMOL_BASE_PARAMS = {
    ...
    "model_name":    "unimolv1",
    "epochs":        25,
    "learning_rate": 2e-4,   # 4× higher than standard 5e-5 → genuinely different model
...
def main() -> None:
    logger.info("=== UniMol Fine-tuning — Third Run (LR=2e-4, unimol2_s3) ===")
    log_device_info()

    out_dir = Path("models/unimol2_s3")
    ...
    logger.info(
        "\nNext: check correlation with models/unimol2 predictions, then add to scripts/11_ensemble.py:\n"
